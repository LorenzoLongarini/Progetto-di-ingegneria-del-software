...
class Fattura(models.Model):
    ...
    cod_fiscale = models.CharField(u'Codice fiscale',max_length=16, default=None, blank=True, null=True)
    cod_cliente = models.CharField(u'Codice cliente',max_length=7, blank=True, null=True)
    #tipo_documento = models.CharField() sempre fattura
    numero = models.IntegerField(validators=[validate_price], unique=True)
...
class TariffaFatt(models.Model):
    ...
    quantita = models.IntegerField(u'Quantità',choices=CHOICES, blank=True, null=True, validators=[validate_price])
    prezzo = models.DecimalField( validators=[validate_price], max_digits=5,decimal_places=2, blank=True, null=True)
